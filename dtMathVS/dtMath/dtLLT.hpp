...
public:
    CdtLLT();
    CdtLLT(const m_type *element, const size_t n_byte);
    CdtLLT(const CdtMatrix<m_row, m_col, m_type> &m);
    CdtLLT(const CdtMatrix3<m_type, m_row, m_col> &m);
    ...
    int8_t Compute(const CdtMatrix<m_row, m_col, m_type> &m);   // Compute Cholesky Decomposition, L*L^T form
    int8_t Compute(const CdtMatrix3<m_type, m_row, m_col> &m);  // Compute Cholesky Decomposition, L*L^T form
    ...
    int8_t Solve(const CdtMatrix<m_row, col, m_type> &b, CdtMatrix<m_col, col, m_type> &x);   // Solve x = (LU)^-1 * b
    int8_t Solve(const CdtVector<m_row, m_type> &b, CdtVector<m_col, m_type> &x);             // Solve x = (LU)^-1 * b
    template <uint16_t col>
    CdtMatrix<m_col, col, m_type> Solve(const CdtMatrix<m_row, col, m_type> &b, int8_t *isOk = nullptr);  // Solve x = (LU)^-1 * b
