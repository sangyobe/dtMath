...
inline CdtSVD<m_row, m_col, m_type>::CdtSVD()
{
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtSVD<m_row, m_col, m_type>::CdtSVD(const m_type *element, const size_t n_byte)
{
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtSVD<m_row, m_col, m_type>::CdtSVD(const CdtMatrix<m_row, m_col, m_type>& m)
{
    if (m_row >= m_col)
    {
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtSVD<m_row, m_col, m_type>::CdtSVD(const CdtMatrix3<m_type, m_row, m_col>& m)
{
    memcpy(m_A, m.m_elem, sizeof(m_type) * m_row * m_col);
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Compute(const m_type *element, const size_t n_byte)
{
    if ((sizeof(m_type) * m_row * m_col) != n_byte)
    {
        m_isOk = 0;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Compute(const CdtMatrix<m_row, m_col, m_type>& m)
{
    if (m_row >= m_col)
    {
        memcpy(m_A, m.m_elem, sizeof(m_type) * m_row * m_col);
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Compute(const CdtMatrix3<m_type, m_row, m_col>& m)
{
    memcpy(m_A, m.m_elem, sizeof(m_type) * m_row * m_col);
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtMatrix<m_row, m_row, m_type> CdtSVD<m_row, m_col, m_type>::GetMatrixU() const
{
    if (m_row > m_col)
    {
        m_type U[m_row * m_row] = { 0, };

        for (int i = 0; i < m_row; i++)
            memcpy(&U[i*m_row], &m_U[i*m_col], sizeof(m_type)*m_col);

        return CdtMatrix<m_row, m_row, m_type>(U);
    }
    else
    {
        return CdtMatrix<m_row, m_row, m_type>(m_U);
    }
}

template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtMatrix<m_row, m_col, m_type> CdtSVD<m_row, m_col, m_type>::GetMatrixS() const
{
    return CdtMatrix<m_row, m_col, m_type>(m_S, 'd');
}

template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtMatrix<m_col, m_col, m_type> CdtSVD<m_row, m_col, m_type>::GetMatrixV() const
{
    if (m_row < m_col)
    {
        m_type V[m_col * m_col] = { 0, };

        for (int i = 0; i < m_col; i++)
            memcpy(&V[i*m_col], &m_V[i*m_row], sizeof(m_type)*m_row);

        return CdtMatrix<m_col, m_col, m_type>(V);
    }
    else
    {
        return CdtMatrix<m_col, m_col, m_type>(m_V);
    }
}

template<uint16_t m_row, uint16_t m_col, typename m_type>
template<uint16_t col>
inline int8_t CdtSVD<m_row, m_col, m_type>::Solve(const CdtMatrix<m_row, col, m_type>& b, CdtMatrix<m_col, col, m_type>& x, m_type tolerance)
{
    int i, j, k, c;
    m_type *pU, *pV;
    m_type sum;
    ...
    if (m_row >= m_col)
    {
        ...
        if (tolerance < sum) tolerance = sum;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Solve(const CdtVector<m_row, m_type>& b, CdtVector<m_col, m_type>& x, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV;
    m_type sum;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
template<uint16_t col>
inline CdtMatrix<m_col, col, m_type> CdtSVD<m_row, m_col, m_type>::Solve(const CdtMatrix<m_row, col, m_type>& b, int8_t * isOk, m_type tolerance)
{
    int i, j, k, c;
    m_type *pU, *pV;
    m_type sum;
    m_type x[m_col*col] = { 0, };
    ...
    {
        *isOk = 0;
        return CdtMatrix<m_col, col, m_type>();
    }
    ...
    return CdtMatrix<m_col, col, m_type>(x);
}

template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtVector<m_col, m_type> CdtSVD<m_row, m_col, m_type>::Solve(const CdtVector<m_row, m_type>& b, int8_t * isOk, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV;
    m_type sum;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Inverse(CdtMatrix<m_col, m_row, m_type>& inv, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV, *pInvA;
    m_type tol;
        ...
        for (i = 0, pV = m_V, pInvA = inv.m_elem; i < m_col; i++, pV += m_col)
        {
            for (j = 0, pU = m_U; j < m_row; j++, pInvA++)
            {
                for (k = 0, *pInvA = 0; k < m_col; k++, pU++)
                    if (m_S[k] > tolerance) *pInvA += *(pV + k) * *pU / m_S[k];
            }
        }
    }
    else
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::Inverse(CdtMatrix3<m_type, m_col, m_row>& inv, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV, *pInvA;
    m_type tol;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline CdtMatrix<m_col, m_row, m_type> CdtSVD<m_row, m_col, m_type>::Inverse(int8_t * isOk, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV, *pInvA;
    m_type tol;
    ...
    {
        *isOk = 0;
        return CdtMatrix<m_col, m_row, m_type>();
    }
    ...
    return CdtMatrix<m_col, m_row, m_type>(m_inv);
}

template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::InverseArray(m_type * inv, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV, *pInvA;
    m_type tol;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline m_type * CdtSVD<m_row, m_col, m_type>::InverseArray(int8_t * isOk, m_type tolerance)
{
    int i, j, k;
    m_type *pU, *pV, *pInvA;
    m_type tol;
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline void CdtSVD<m_row, m_col, m_type>::HouseholdersReductionToBidiagonalForm_Mxn()
{
    int i, j, k, ip1;
    ...
    for (i = 0, pUi = m_U, ip1 = 1; i < m_col; pUi += m_col, i++, ip1++)
    {
        ...
        if (scale > std::numeric_limits<m_type>::epsilon())
        {
            ...
            *(pUi + ip1) -= s;
            ...
            for (k = ip1; k < m_col; k++) *(pUi + k) *= scale;
        }
    }
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline int8_t CdtSVD<m_row, m_col, m_type>::GivensReductionToDiagonalForm_Mxn()
{
    m_type epsilon;
    m_type c, s;
    m_type f, g, h;
    ...
    for (k = m_col - 1; k >= 0; k--)
    {
        ...
        while (1)
        {
            ...
            for (m = k; m >= 0; m--)
            {
                ...
                if (std::abs(m_S[m - 1]) <= epsilon) break;
            }
...
template<uint16_t m_row, uint16_t m_col, typename m_type>
inline void CdtSVD<m_row, m_col, m_type>::SortByDecreasingSingularValues_Mxn()
{
    int i, j, maxIdx;
    m_type temp;
    m_type *pM1, *pM2;
    ...
    for (i = 0, pVi = m_V, ip1 = 1; i < m_row; pVi += m_row, i++, ip1++)
    {
        ...
        if (scale > std::numeric_limits<m_type>::epsilon())
        {
            ...
            *(pVi + ip1) -= s;
            ...
            for (k = ip1; k < m_row; k++) *(pVi + k) *= scale;
        }
    }
