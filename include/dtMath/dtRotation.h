...
public:
    ...
    static Rotation expMap(Matrix3<t_type, t_row, t_col> &skew); // Exponential map, so(3) -> SO(3)
    static Rotation ExpMap(Vector3<t_type, 3> &v);               // Exponential map, R3 -> SO(3)
    Matrix3<t_type, t_row, t_col> log() const;                   // Logarithm of rotation matrix
    Matrix3<t_type, t_row, t_col> logMap() const;                // Logarithmic map, log(R) = ln(R) : SO(3) -> so(3), SO(3) is Special Orthogonal Group, so(3) is the set of skew-symmetric 3x3 matrices
    Vector3<t_type, 3> LogMap() const;                           // Logarithmic map, Log(R) = u*phi : SO(3) -> R3, u is vector and phi is scalar
